...
def get_nearest_neighbors(E, estimate_vector):
    # Compute distances and use heapq.nsmallest to get the 6 smallest distances
    distance_map = [((E - float(val)) ** 2, idx) for idx, val in enumerate(estimate_vector)]
    nearest_neighbors = heapq.nsmallest(6, distance_map, key=lambda x: x[0])
    
    # Return the indices of the 6 closest values
    return [idx for dist, idx in nearest_neighbors]
